// client/src/features/analytics/ProjectAnalyticsDashboard.tsx
...
import TeamHeadcountMap from "@/features/team/TeamHeadcountMap";
import AIProjectAnalyticsTab from "@/features/analytics/AIProjectAnalyticsTab";
import { EnvironmentalContextCard } from "@/features/environment/environmental-context-card";
import { EarthNullschoolGlobe } from "@/features/environment/EarthNullschoolGlobe";
...
type Incident = {
  ...
  projectId?: string;
  scientificAnalysis?: IncidentAnalysisResult & { source?: string };
};
...
const ProjectAnalyticsDashboard: React.FC<ProjectAnalyticsDashboardProps> = ({
  ...
  const exportToPDF = () => {
    const doc = new jsPDF();
    const analyzedIncidents = incidents.filter((incident) => incident.scientificAnalysis);
    const highRiskIncidents = analyzedIncidents.filter((incident) =>
      ["HIGH", "CRITICAL"].includes(incident.scientificAnalysis?.risk.band ?? "")
    );
  ...
  const exportToExcel = () => {
    ...
    const incidentRows = incidents.map((incident) => ({
      ...incident,
      scientificAnalysis: undefined,
      riskBand: incident.scientificAnalysis?.risk.band ?? "",
      riskScore: incident.scientificAnalysis?.risk.score ?? "",
      riskConfidence: incident.scientificAnalysis?.risk.confidence ?? "",
      fieldMode: incident.scientificAnalysis?.lowResourcePlan.mode ?? "",
      analysisVersion: incident.scientificAnalysis?.analysisVersion ?? "",
      analysisGeneratedAt: incident.scientificAnalysis?.generatedAt ?? "",
      analysisSource: incident.scientificAnalysis?.source ?? "",
    }));
                      ...
                      {incidents.map((i) => (
                        <tr key={i.id} className="border-b">
                          ...
                          <td className="px-3 py-2">
                            {i.scientificAnalysis
                              ? `${i.scientificAnalysis.risk.band} (${i.scientificAnalysis.risk.score}/100)`
                              : "—"}
                          </td>
