...
const ACKED_EMERGENCIES_STORAGE_KEY = "hydrosafe:acknowledged-emergencies";

function readAcknowledgedEmergencyIds(): string[] {
  try {
    const raw = localStorage.getItem(ACKED_EMERGENCIES_STORAGE_KEY);
...
function persistAcknowledgedEmergencyIds(ids: Set<string>) {
  localStorage.setItem(ACKED_EMERGENCIES_STORAGE_KEY, JSON.stringify(Array.from(ids)));
}
...
export function useProjectEmergencyAlarm(options: UseProjectEmergencyAlarmOptions) {
  ...
  const acknowledgedEmergencies = useRef<Set<string>>(new Set(readAcknowledgedEmergencyIds())); // Track emergencies we've acknowledged
  ...
  const acknowledge = useCallback(async () => {
    ...
    try {
      ...
      const result = await writeOrQueueFirestoreDoc({
        kind: "emergency_ack",
        collectionPath: `emergencies/${activeEmergency.id}/acks`,
        docId: user.uid,
        data: ackData,
      });
      ...
      persistAcknowledgedEmergencyIds(acknowledgedEmergencies.current);
      ...
      console.log(
        result.queued
          ? `✅ Emergency ${activeEmergency.id} acknowledgment queued by ${user.uid}`
          : `✅ Emergency ${activeEmergency.id} acknowledged successfully by ${user.uid}`
      );
