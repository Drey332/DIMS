...
export function CommandDashboard() {
  ...
  return (
    <div className="px-2 md:px-3 pb-8 max-w-[1440px] mx-auto glassy-bg grid-pattern">
      {/* --- Filter Controls --- */}
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-7 mb-8">
        ...
        <div className="xl:col-span-2 space-y-8">
          {/* --- Emergencies --- */}
          <Card className="hydro-card glassy-card animate-glow">
            <CardHeader className="flex flex-row items-center justify-between pb-4">
              <CardTitle className="text-2xl font-black holographic flex items-center gap-2">
                <AlertTriangle className="text-orange-500 animate-glow" />
                {incidentView === "ACTIVE"
          ...
          {/* --- Observations --- */}
          <Card className="hydro-card glassy-card mt-6 animate-glow">
            <CardHeader>
              <CardTitle className="text-xl font-bold holographic flex items-center">
                <Eye className="text-blue-500 mr-3 h-5 w-5 animate-glow" />
                {obsView === "OPEN"
                  ? "Open Observations"
                  : obsView === "CLOSED"
                  ? "Closed Observations"
      ...
      <Card className="hydro-card glassy-card animate-glow">
      <CardHeader>
      <CardTitle className="text-xl font-bold holographic flex items-center gap-2">
        <Users className="text-primary animate-glow" />
        Command Team Status
      </CardTitle>
      </CardHeader>
      <CardContent>
      ...
      {viewIncident && (
      <div className="fixed inset-0 glassy-bg z-50 flex items-center justify-center" style={{ backdropFilter: "blur(8px)" }}>
      <div className="glassy-card p-8 max-w-4xl w-full relative animate-glow">
      <button className="absolute right-4 top-4 text-gray-400 hover:text-red-600" onClick={() => setViewIncident(null)}>
      <X className="w-6 h-6" />
      </button>
      <h2 className="text-2xl font-bold holographic mb-4 flex items-center">
      <AlertTriangle className="text-orange-500 mr-2 animate-glow" />
      {viewIncident.title}
      </h2>
      ...
      <h3 className="font-bold text-xl mb-2 flex items-center neon-text">
        <Users className="mr-2 text-blue-500 animate-glow" /> Headcount Map: Mustered Personnel
      </h3>
      <div className="w-full">
        {/* Pass full props for analytics/replay! */}
        <TeamHeadcountMap
          acks={acks}
          teamMembers={teamMembers}
          // Convert startTime string (ISO) to number (ms since epoch)
          incidentStartTime={
            viewIncident.startTime
              ? new Date(viewIncident.startTime).getTime()
              : Date.now() // fallback to "now" so it's always a number
          }
          enableReplay={true}
          replayWindow={5 * 60 * 1000} // 5 minutes replay window for emergency incidents
        />
      </div>
      {/* Acknowledged List */}
      <div className="mt-5">
