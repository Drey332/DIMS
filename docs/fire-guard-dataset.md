...
- **Historic Offshore Benchmarks** *(ships with the platform today)*: Piper Alpha (1988) and Deepwater Horizon (2010) now live in `data/fire-incidents.seed.json` and conform to `shared/fire-intel/schema.ts`. `server/fire-guard-harvester.ts` loads them as the default static source, while `server/fire-intel/ingest.ts` validates, persists, and vectorizes the same records for advisor prompts.
...
- Remote connectors (CSB, BSEE, NFIRS) remain available, but you must explicitly pass their IDs in the `sources` array when triggering a harvest to avoid failed fetches in constrained environments.
...
- Operationalize the Fire Guard Harvester (`server/fire-guard-harvester.ts`) with the built-in historic offshore dataset first, then progressively add remote sources (CSB, BSEE, NFIRS) by supplying their IDs to `/api/fire-guard/harvest`. Each harvest snapshot can be analyzed through `/api/fire-guard/analyze` for explainable pattern intelligence stored alongside the lake.
